document.addEventListener("DOMContentLoaded", () => {
  ...
  const API_URL = "http://localhost:3001/films"; // Adjusted to your provided endpoint

  // Fetch and display the movie list
  fetch(API_URL)
    .then(response => response.json())
    .then(movies => {
      loadingPlaceholder.style.display = "none"; // Hide the loading placeholder once data is fetched
  ...
  function handleBuyTicket(movie) {
    ...
    if (ticketsAvailable > 0) {
      ...
      if (movie.capacity - movie.tickets_sold === 0) {
        ...
        // Update movie list to show sold-out state
